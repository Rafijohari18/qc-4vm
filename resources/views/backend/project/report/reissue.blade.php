...
<div class="container-fluid  container-p-y">
    ...
    <form action="{{ route('report.update.occurences', ['id' => $data['report']['id']]) }}" method="POST" enctype="multipart/form-data">
          @csrf
          @method('PUT')
    <div class="card">
      <div class="card-header">
        <div class="title-heading">
              <h6 class="m-0">Reissue Report </h6>
        </div>
      </div>
        ...
        <div class="card-body">
              ...
              <div class="col-md-10">
                <select name="project_id" id="project_id" class="form-control" readonly > 
                  ...
                  @foreach($data['project'] as $project)
                    <option readonly value="{{ $data['report']->project_id }}" {{ $data['report']->project_id  == $project->Project->id  ? 'selected' : ''}}>{{ $project->Project->name }}</option>
                  @endforeach
                </select>
              </div>
            </div>
         ...
         <div class="form-group">
            <div class="row">
              ...
              <div class="col-md-10">
                  <select class="form-control" name="module_id" id="module_id" readonly >
                     <option selected disabled>Pilih Modul</option>

                      @foreach ($data['modul'] as $modul)
                          <option value="{{ $data['report']->module_id }}" {{ $data['report']->module_id  == $modul->id  ? 'selected' : ''}}> {{ $modul['name'] }}</option>
                      @endforeach
                  </select>
              </div>
            </div>
        ...
        <div class="form-group">
          <div class="row">
              ...
              <div class="col-md-10">
                <input type="text" readonly required class="form-control @error('code') is-invalid @enderror" name="code" value="{{ $data['report']->code }}"  placeholder="enter code...">
              </div>
          </div>
      </div>
          ...
          <div class="form-group">
            <div class="row">
              ...
              <div class="col-md-10">
                  <select class="form-control" name="priority" required readonly>
                     <option selected disabled>Pilih Priority</option>
                     <option value="{{ $data['report']->priority }}" {{ $data['report']->priority  == 0  ? 'selected' : ''}} >Low/Improvemen</option>
                     <option value="{{ $data['report']->priority }}" {{ $data['report']->priority  == 1  ? 'selected' : ''}} >Visual Error</option>
                     <option value="{{ $data['report']->priority }}" {{ $data['report']->priority  == 2  ? 'selected' : ''}} >Bug</option>
                     <option value="{{ $data['report']->priority }}" {{ $data['report']->priority  == 3  ? 'selected' : ''}} >Error</option>
                     <option value="{{ $data['report']->priority }}" {{ $data['report']->priority  == 4  ? 'selected' : ''}} >Process-breaking Error</option>
                  </select>
              </div>
            </div>
          </div>
          ...
          <div class="form-group">
            <div class="row">
              ...
              <div class="col-md-10">
                <input type="text" value="{{ $data['report']->url }}"  readonly class="form-control @error('url') is-invalid @enderror" name="url"  placeholder="enter url...">
          ...
          <div class="form-group">
            <div class="row">
              ...
              <div class="col-md-10">
              <textarea name="reproduction_steps" class="form-control" id="tinymce" readonly disabled>{{ $data['report']->reproduction_steps }}</textarea>
              </div>
            </div>
          </div>
          ...
          <div class="form-group">
            <div class="row">
              ...
              <div class="col-md-10">
              <textarea name="extra_note" class="form-control" id="tinymce2" ></textarea>
              </div>
            </div>
          </div>
        ...
        <div class="form-group">
          <div class="row">
              ...
              <div id="inputFormRow" class="row">
                  @if($data['report']->attachments != null)
                  
                  @for($i=0; $i < count($data['report']->attachments) ; $i++)
                  @php
                  $extension = pathinfo(storage_path($data['report']->attachments[$i]), PATHINFO_EXTENSION);
                  @endphp
                  <input type="hidden" name="attachments_dulu[]" value="{{ $data['report']->attachments[$i] }}">
                  
                 
                    <div class="col-md-11">
                          @if($extension == 'jpg' || $extension == 'JPG' || $extension == 'png' || $extension == 'gif')
                          <img src="{{ asset($data['report']->attachments[$i]) }}" class="img-thumbnail mb-5 w-50">
                          @elseif($extension == 'mp4')
                          <video width='100%' height='600px' controls>
                            <source src="{{ $data['report']->attachments[$i] }}" type="video/mp4">
...
<script type="text/javascript">
  ...
  $("#addRow").click(function () {
      ...
      html += ' <input type="file" name="extra_attachments[]" class="form-control mb-3">';
      html += '</div>';
